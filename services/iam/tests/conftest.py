"""Pytest configuration shared by every IAM test.

The platform's :class:`aidp_common.config.Settings` requires ``AIDP_DB_URL``,
``AIDP_REDIS_URL`` and ``AIDP_SERVICE_NAME`` to be set. The IAM service
...
module imports ``aidp_iam.main``). The autouse fixture then
...
# ``create_app()`` call in :mod:`aidp_iam.main` finds a valid settings
...
os.environ.setdefault("AIDP_SERVICE_NAME", "aidp-iam-test")
...
os.environ.setdefault(
    "AIDP_JWT_SECRET",
    "aidp-iam-test-secret-do-not-use-in-prod-padded-to-64-bytes-min!!",
)
...
_TEST_ENV: dict[str, str] = {
    ...
    "AIDP_SERVICE_NAME": "aidp-iam-test",
    "AIDP_KAFKA_BROKERS": "localhost:9092",
    "AIDP_JWT_SECRET": "aidp-iam-test-secret-do-not-use-in-prod-padded-to-64-bytes-min!!",
}


@pytest.fixture(autouse=True)
def _aidp_iam_env(monkeypatch: pytest.MonkeyPatch) -> Generator[None, None, None]:
