"""Alembic environment for the AIDP IAM service.

This module is the bridge between Alembic and the IAM service's
...
- We import :mod:`aidp_iam.models` for its side effect of registering
  every table on :class:`aidp_iam.models.Base` — Alembic then walks
...
- We set ``compare_server_default=True`` so default-value changes
  (e.g. ``default="active"`` on ``status``) show up in diffs.
"""
...
# ---------------------------------------------------------------------------
# Make the IAM package importable so ``aidp_iam.models`` resolves when
# Alembic is run from the service root (``cd services/iam && alembic ...``).
# ---------------------------------------------------------------------------
...
# Importing ``aidp_iam.models`` registers every IAM table on ``Base.metadata``.
import aidp_db.session  # noqa: E402, F401  # side-effect: L1 listener
from aidp_iam.models import Base  # noqa: E402
...
_LOG = logging.getLogger("alembic.env.iam")

# Metadata that autogenerate compares against. Pointed at the IAM
# service's local ``Base`` so the diff is scoped to the IAM schema
