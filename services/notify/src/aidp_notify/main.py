"""FastAPI application factory for the Notify service.

This module is the entry point for ``uvicorn aidp_notify.main:app``. It:
...
- Runs Alembic migrations to ``head`` on startup so a fresh
...
The HTTP routes for actual notification operations (send, template
CRUD, channel CRUD, log query) live in :mod:`aidp_notify.api.send` and
:mod:`aidp_notify.api.templates`.
"""
...
# :mod:`aidp_notify.models` so the notify metadata is registered on
# the ``Base`` before Alembic autogenerate is ever pointed at it.
...
from aidp_notify import models  # noqa: F401  # side-effect: register tables

#: Absolute path to this service's Alembic script directory.
#:
#: Derived from ``__file__`` rather than CWD so the same code path
#: works under (a) a container image whose WORKDIR is the repo root,
#: (b) monorepo-root ``uv run pytest``, and (c) the local ``uv run
#: uvicorn`` dev loop. The layout is::
#:
#:     services/notify/src/aidp_notify/main.py   <- this file
#:     services/notify/src/aidp_notify/
#:     services/notify/src/
#:     services/notify/
#:     services/notify/alembic/               <- Alembic scripts
#:
#: so ``Path(__file__).parent.parent.parent`` lands at
#: ``services/notify/``. ``aidp_db.migration.run_migrations`` will
#: further honour ``AIDP_ALEMBIC_SCRIPT_LOCATION`` when set, which
#: lets ops override the location at deploy time without rebuilding
#: the image.
...
async def lifespan(_app: FastAPI) -> AsyncIterator[None]:
    ...
    3. Run Alembic migrations to ``head``. Any failure is logged and
       re-raised so the platform orchestrator's startup probe catches
       it and marks the pod as failing.
    ...
    _LOG.info("notify service starting", extra={"service": settings.service_name})
    try:
        ...
        raise UpstreamError("notify service startup failed", details={"error": str(exc)}) from exc

    try:
        yield
    finally:
        # Dispose the engine so its connection pool shuts down
        # cleanly.
        try:
            dispose_engine()
        except Exception:  # pragma: no cover - best-effort cleanup
            _LOG.exception("error while disposing engine")
        _LOG.info("notify service stopped")
...
def create_app() -> FastAPI:
    """Construct the FastAPI app for the Notify service.
    ...
    Returns:
        ...
        ``uvicorn aidp_notify.main:app`` or as a child in a larger
        composition root.
    """
    settings = get_settings()
    app = FastAPI(
        title="AIDP Notify Service",
        version="0.1.0",
        description=(
            "Multi-channel notification (email / feishu / webhook / sms) "
            "with Handlebars template rendering and a persistent send log."
        ),
        lifespan=lifespan,
    )
    ...
    # Mount the notify API. The router is imported lazily so the
    # create_app() call stays fast at import time.
    from aidp_notify.api.errors import install_app_error_handler
    from aidp_notify.api.send import router as send_router
    from aidp_notify.api.templates import router as templates_router
    ...
    _LOG.info(
        "notify app created",
        extra={"service": settings.service_name, "port": 8003},
    )
    return app


# Module-level instance so ``uvicorn aidp_notify.main:app`` works
# without an extra factory import. The factory remains available for
# tests that want to construct the app with a custom lifespan.
...
# aidp_notify.main`` (which does not go through ``create_app``) still
# emits structured logs. This is a no-op if :func:`setup_logging` is
# invoked again from the lifespan.
if not logging.getLogger().handlers:  # pragma: no cover - import-time fallback
    setup_logging(service_name="aidp-notify")
